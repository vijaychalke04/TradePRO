...
app.use(cors({
  origin: function(origin, cb){
    ...
    if(!ALLOWED_ORIGINS.length) return cb(null, true); // backwards compatible
...
// =============================================
const CONFIG = {
  // SECURITY: do NOT hardcode secrets in code. Keep them in Render env vars.
...
if(!CONFIG.apiKey || !CONFIG.clientId){
  // Fail fast so the deployment clearly shows missing env variables.
  const msg = "Missing required env vars: ANGEL_API_KEY and/or ANGEL_CLIENT_ID";
  console.error(msg);
  // On Render/production, crashing is better than running with broken config.
...
// =============================================
// TIME HELPERS (IST)
// Render often runs in UTC; Angel candle APIs expect local market timestamps.
// We use IST (UTC+05:30) consistently.
// =============================================
function nowIST(){
  const d = new Date();
...
function fmtAngelDateTime(d){
  // YYYY-MM-DD HH:mm (using UTC fields on an IST-shifted Date)
...
// INSTRUMENTS
// =============================================
// MCX tokens are dynamically resolved to nearest-expiry FUT contracts at runtime.
// Seed with safe defaults; will be replaced by resolveMctxTokens().
...
// NOTE: Do not use COMMODITIES (static) for MCX. Always use COM_TOKENS.
...
function suggestOption(name, ltp, direction, isIndex){
  ...
  // BUY => suggest CE; SELL => suggest PE (for directional hedge)
...
// Throttle disk writes to avoid excessive IO on frequent quote polling.
let _quoteWriteTimer = null;
let _lastQuoteWriteAt = 0;
const QUOTE_WRITE_MIN_GAP_MS = 10_000; // at most once per 10s
...
function cacheQuotes(fetchedList, exchange) {
  ...
  // Debounced write (best-effort)
  const scheduleWrite = ()=>{
    try{
      const delta = Date.now() - _lastQuoteWriteAt;
      if(delta < QUOTE_WRITE_MIN_GAP_MS) return; // already wrote recently
      _lastQuoteWriteAt = Date.now();
      fs.writeFile(QUOTE_CACHE_PATH, JSON.stringify(QUOTE_CACHE, null, 2), ()=>{});
    }catch(e){}
  };

  if(_quoteWriteTimer) clearTimeout(_quoteWriteTimer);
  _quoteWriteTimer = setTimeout(scheduleWrite, 250);
}
...
app.post("/api/angel", async(req,res)=>{
  ...
  try{
    ...
    if(action==="fetch_all"){
      ...
      if(!effectiveToken){
        return res.status(401).json({ success:false, error:"Not logged in (missing token)" });
      }

      const [indRaw, comRaw, stkRaw] = await Promise.all([
        fetchQuotes(effectiveToken,"NSE",NSE_INDICES),
        fetchQuotes(effectiveToken,"MCX",COM_TOKENS),
        fetchQuotes(effectiveToken,"NSE",STOCKS)
      ]);
      ...
      const indices = await Promise.all(indRaw.map(async x=>{
        ...
        try{
          ...
          const now = nowIST();
          const todate = fmtAngelDateTime(now);
          const from = new Date(now.getTime() - 2*60*60*1000);
          const fromdate = fmtAngelDateTime(from);

          const candles = await fetchCandleData(effectiveToken,"NSE",d.token, "FIVE_MINUTE", fromdate, todate);
          ...
          const mc = await getMonteCarloCached(effectiveToken, "NSE", d.token, "FIFTEEN_MINUTE", 20, 1000);
          const edgeProb = sig.signal === "BUY" ? mc?.stats?.winUp0_5 : sig.signal === "SELL" ? mc?.stats?.winDown0_5 : null;
          const edge = classifyEdge(edgeProb);
...
async function getMonteCarloCached(jwtToken, exchange, token, interval="FIFTEEN_MINUTE", days=20, paths=1000){
  ...
  const nowD = nowIST();
  const todate = fmtAngelDateTime(nowD);
  const from = new Date(nowD.getTime() - 14*24*60*60*1000);
  const fromdate = `${fmtAngelDateTime(from).slice(0,10)} 09:15`;
