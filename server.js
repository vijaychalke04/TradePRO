...
function runBacktest(candles, fastP = 9, slowP = 21) {
  ...
  const bb = bollinger(cl); const mc = macd(cl); const st = supertrend(candles); const pats = detectPatterns(candlesFinal);
...
app.post("/api/backtest/montecarlo", async (req, res) => {
  try {
    ...
    const closes = candlesFinal.map(c => c.c).filter(isFinite);
...
const istDate = (d = new Date()) => new Date(d.getTime() + IST_OFFSET_MS);
const fmtDT = (d) => `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2,"0")}-${String(d.getDate()).padStart(2,"0")} ${String(d.getHours()).padStart(2,"0")}:${String(d.getMinutes()).padStart(2,"0")}`;

const makeTimeRange2h = () => {
  // Angel historical API expects IST timestamps; Render runs in UTC
  const now = istDate(new Date());
  const to2 = fmtDT(now);
  const fr2 = istDate(new Date(Date.now() - 2 * 3600000));
  const from2 = fmtDT(fr2);
  return { from2, to2 };
};
...
// Fallback: if intraday window returns nothing, try today's regular market window (IST)
let candles2 = candles;
if (!candles2.length) {
  const nowI = istDate(new Date());
  const day = `${nowI.getFullYear()}-${String(nowI.getMonth()+1).padStart(2,"0")}-${String(nowI.getDate()).padStart(2,"0")}`;
  candles2 = await fetchCandles(token, exch, d.token, "FIFTEEN_MINUTE", `${day} 09:15`, `${day} 15:30`);
}
const candlesFinal = candles2;

          if (!candlesFinal.length) console.log("Candles empty for", exch, d.symbol, d.token, "from", from2, "to", to2);
           const closes = candlesFinal.map(c => c.c);
          const r = rsi(closes) ?? null;
          const mc = macd(closes) ?? null;
          const atrPack = calcATR(candlesFinal, 14);
          const regime = getRegime(candlesFinal);
          const pats = detectPatterns(candlesFinal);
          const aw = candlesFinal.length ? antiWhipsaw(candlesFinal) : { signal: "HOLD", reason: "No candles" };
          const ind = {
          ...
          out._levels = computeLevelsFromCandles(candlesFinal, 12);
