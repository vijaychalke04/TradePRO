...
// Health check — responds immediately so Render knows we're alive
...
function requestAngel(rp, method, headers, data) {
  return new Promise((resolve, reject) => {
    const req = https.request({ hostname: CONFIG.baseURL, path: rp, method, headers, timeout: 8000 }, res => {
    ...
    }); req.setTimeout(8000, () => req.destroy(new Error("Angel API timeout 8s"))); req.on("error", reject); if (data) req.write(JSON.stringify(data)); req.end();
  });
}
...
async function fetchCachedCandles(jwt, exchange, symbolToken, interval, fromdate, todate) {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    try { fresh = await fetchCandles(jwt, exchange, symbolToken, interval, fromdate, todate); if (fresh.length) break; } catch(e) { if (attempt === 0) await new Promise(r => setTimeout(r, 400)); }
  }
...
function enrichEngine(m, eng) {
  ...
  // Dynamic risk sizing — WIDER targets for meaningful trades
  const atr = +(eng?.atr || 0);
  const base = atr > 0 ? atr * 1.5 : (ltp * (mode === "MODE2" ? 0.016 : 0.012));
  const slMult = mode === "MODE2" ? 1.0 : 0.85;
  const t1Mult = mode === "MODE2" ? 2.2 : 1.8;
  const t2Mult = mode === "MODE2" ? 3.5 : 2.8;
  ...
  // Confidence % and friendly mode label
  const confPct = Math.min(95, Math.round(50 + Math.abs(score - 50) * 1.2));
  const modeLabel = mode === "MODE2" ? "Active" : "Safe";
...
const nseIdx = await Promise.all(indRaw.map(x => processInstrument(x, "NSE", { isIndex: true })));
      ...
      // ═══ PERSISTENT NOTIFICATIONS + BUY/SELL TELEGRAM ═══
      if (!global._NOTIFS) global._NOTIFS = [];
      // Store READY alerts
      for (const al of alerts) {
        if (al.category !== "READY" && al.status !== "READY") continue;
        const exists = global._NOTIFS.find(n => n.key === `${al.name}:${al.type}` && Date.now() - n.ts < 600000);
        if (!exists) global._NOTIFS.unshift({ key: `${al.name}:${al.type}`, title: `🚨 ${al.name} — ${al.label||al.type}`, body: `${al.action||""} | ₹${fmtR(al.trigger||0)} | T ₹${fmtR(al.target1||0)} | SL ₹${fmtR(al.sl||0)} | ${al.optionHint||""}`, type: "alert", ts: Date.now() });
      }
      // Push BUY/SELL trade signals to Telegram + store
      const TRADE_CD = global._TG_TRADE_CD || (global._TG_TRADE_CD = {});
      for (const it of [].concat(indices||[], stocks||[], commodities||[])) {
        const sig = (it.engine?.signal||"").toUpperCase();
        if (sig !== "BUY" && sig !== "SELL") continue;
        const cdKey = `trade:${it.name}:${sig}`;
        if (TRADE_CD[cdKey] && Date.now() - TRADE_CD[cdKey] < 600000) continue;
        TRADE_CD[cdKey] = Date.now();
        const e = it.engine||{};
        sendTg(`📊 ${sig} — ${it.name}\n${e.trade||sig}\nOpt: ${e.optionSuggestion?.suggested||"—"}\n₹${it.ltp} | T ₹${e.projectedTarget||"—"} | SL ₹${e.stopLoss||"—"}\nConf: ${e.confPct||50}%`);
        global._NOTIFS.unshift({ key: cdKey, title: `📊 ${sig} — ${it.name}`, body: `${e.trade||sig} | ₹${it.ltp} | T ₹${e.projectedTarget||"—"} | SL ₹${e.stopLoss||"—"} | ${e.optionSuggestion?.suggested||""}`, type: sig.toLowerCase(), ts: Date.now() });
      }
      if (global._NOTIFS.length > 150) global._NOTIFS.length = 100;
...
// ═══ NOTIFICATIONS API ═══
app.get("/api/notifications", (_, res) => res.json({ ok: true, items: global._NOTIFS || [] }));
...
app.listen(PORT, () => console.log(`🚀 Trade Genie PRO v3.5 on port ${PORT}`));
