...
function scoreEngine(m, ind, ctx = {}) {
  ...
  // If any gate failed, force HOLD unless score is extreme
  ...
  if (!hasHardGate) {
    ...
    // Still allow signal on very strong conviction despite gates
  ...
  const confidence = score >= 80 || score <= 20 ? "HIGH" : score >= 65 || score <= 35 ? "MED" : "LOW";
...
function buildSmartAlerts(universe, ctxByKey = {}) {
  ...
  for (const m of (universe || [])) {
    ...
    // Try each detector; pick the best one per instrument
    const candidates = [
    ...
    if (action === "fetch_all") {
      ...
      const processInstrument = async (x, exch, ctx = {}) => {
        ...
        try {
          ...
          // Combine anti-whipsaw + score (anti-whipsaw provides direction confirmation)
          let finalSig = (se.signal || "HOLD").toUpperCase();
          const awSig = (aw.signal || "HOLD").toUpperCase();
          if (awSig === "HOLD" && (aw.reason === "Not enough candles" || aw.reason === "No candles")) {
            // Not enough data for anti-whipsaw — fall through to score engine
            finalSig = se.signal;
          } else if (awSig === "HOLD") {
            // Anti-whipsaw says no clear direction — only override if score is weak
            if (se.score >= 72 || se.score <= 28) {
              finalSig = se.signal; // Strong score overrides hold
            } else {
              finalSig = "HOLD";
              se.reasonSimple = aw.reason || se.reasonSimple;
            }
          } else {
            // Anti-whipsaw has a direction — confirm with score
            if (awSig === "BUY" && se.score < 55) { finalSig = "HOLD"; se.reasonSimple = "Wait — move not strong enough yet."; }
            else if (awSig === "SELL" && se.score > 45) { finalSig = "HOLD"; se.reasonSimple = "Wait — down move not strong enough yet."; }
            else finalSig = awSig;
          }
...
const nseIdx = await Promise.all(indRaw.map(x => processInstrument(x, "NSE", { isIndex: true })));
      ...
      return res.json({ ok: true, data: { indices: strip(indices), commodities: strip(commodities), stocks: strip(stocks), watchlist, alerts, mcxResolved: RESOLVED_MCX?.picked?.map(p => p.label + "=" + p.symbol) || [] } });
    }
