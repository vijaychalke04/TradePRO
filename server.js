...
function enrichEngine(market, engine){
  ...
  const isIndex = String(market?.exchange || "").toUpperCase()==="NSE" && String(market?.name||"").toUpperCase().includes("NIFTY");

  if(signal==="BUY"){
    trade = isIndex ? "BUY CALL (Index)" : `BUY ${market.name}`;
    projectedTarget = ltp ? +(ltp*(1+tgtPct)).toFixed(2) : null;
    stopLoss = ltp ? +(ltp*(1-slPct)).toFixed(2) : null;
  }else if(signal==="SELL"){
    trade = isIndex ? "BUY PUT (Index)" : `SELL ${market.name}`;
    projectedTarget = ltp ? +(ltp*(1-tgtPct)).toFixed(2) : null;
    stopLoss = ltp ? +(ltp*(1+slPct)).toFixed(2) : null;
  }else{
    trade = "WAIT";
    projectedTarget = ltp ? +ltp.toFixed(2) : null;
    stopLoss = ltp ? +ltp.toFixed(2) : null;
  }
