...
// MCX live LTP for resolved tokens
app.get("/api/mcx/ltp", async (req,res)=>{
  try{
    if(!ANGEL_TOKEN) return res.status(401).json({ success:false, error:"Not logged in. Call /api/angel?action=login first." });

    const picked = (RESOLVED_MCX && Array.isArray(RESOLVED_MCX.picked)) ? RESOLVED_MCX.picked : [];
    const tokens = picked.map(p=>String(p.token)).filter(Boolean);
    if(tokens.length===0) return res.json({ success:false, error:"No MCX tokens resolved yet", resolved: RESOLVED_MCX });

    const q = await fetchQuotes(ANGEL_TOKEN, "MCX", tokens);

    // Normalize response -> array
    const feed = q && q.data && q.data.fetched ? q.data.fetched : (q && q.data ? q.data : q);
    const rows = Array.isArray(feed) ? feed : [];

    const byToken = new Map(rows.map(r=>[String(r.symbolToken||r.token||""), r]));
    const out = picked.map(p=>{
      ...
      return {
        ...
        low: num(r.low),
        close: num(r.close),
        changePct: (r.netChange||r.netChangePercent||r.changePct||r.percentChange)
      };
    });

    res.json({ success:true, resolved: { updatedAt: RESOLVED_MCX.updatedAt, picked }, data: { commodities: out } });
  }catch(e){
