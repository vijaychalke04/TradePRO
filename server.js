// =============================================
// TRADE GENIE PRO - PRODUCTION SERVER
...
const Greeks = {
  ...
  compute: ({ S, K, T, r = 0.07, sigma = 0.2, type = "CE" }) => {
    ...
    return {
      ...
      vega: +vega.toFixed(4), theta: +theta.toFixed(4)
    };
  }
};
...
const STK_TOKENS = ["3045", "11536", "1333", "1594", "4963", "1660", "3787"];
  ...
  try {
    ...
    const parseExpiry = (e) => {
      try {
        e = String(e || "").trim();
        if (!e) return null;
        if (e.includes("-")) { const d = new Date(e); return isNaN(d) ? null : d; }
...
function signalEngine(i, commodity = false) {
  ...
  if (!beta) return { signal: "HOLD", mode, risk, warning: "Structure weak (blocked)" };
  return {
    signal: alpha, mode, risk,
...
function applyConfirmationBuffer(token, rawSignal) {
  ...
  if (s === st.committed) {
    st.pending = s; st.pendingCount = 0; st.lastTs = Date.now();
    SIGNAL_BUFFER.byToken[t] = st;
    return { signal: st.committed, stabilizing: false };
  }

  if (s === st.pending) { st.pendingCount += 1; }
  else { st.pending = s; st.pendingCount = 1; }

  let stabilizing = true;
  if (st.pendingCount >= SIGNAL_BUFFER.confirmTicks) {
    st.committed = st.pending; st.pendingCount = 0; stabilizing = false;
  }
  st.lastTs = Date.now();
  SIGNAL_BUFFER.byToken[t] = st;
...
function suggestOption(name, ltp, direction, isIndex) {
  ...
  let strike = roundToStep(price, step);
  if (optType === "CE" && strike < price) strike += step;
  if (optType === "PE" && strike > price) strike -= step;

  const cleanName = String(name || "").replace(/-EQ$/i, "").trim();
  return {
    optionType: optType, strike, step,
    suggested: `${cleanName} ${strike} ${optType}`,
...
function enrichEngine(market, engine) {
  ...
  const tgtPct = mode === "MODE2" ? 0.020 : 0.010;
  const slPct = mode === "MODE2" ? 0.010 : 0.005;
  ...
  return {
    ...
    trade, projectedTarget, targetLow, targetHigh, stopLoss, optionSuggestion,
...
function getMarketRegime(candles) {
  ...
  for (let i = 1; i < candles.length; i++) {
    ...
    const tr = Math.max(candles[i].h - candles[i].l, Math.abs(candles[i].h - prevClose), Math.abs(candles[i].l - prevClose));
    trs.push(tr);
  }
...
function computeMCFromCloses(closes, days, paths) {
  ...
  return {
    ok: true, S0, mu, sigma: sd,
    params: { days: nSteps, paths: nPaths },
    stats: {
      winUp0_5: +(winsUp / nPaths * 100).toFixed(2),
      winDown0_5: +(winsDn / nPaths * 100).toFixed(2),
      p10: +pct(0.10).toFixed(2), p50: +pct(0.50).toFixed(2), p90: +pct(0.90).toFixed(2),
      mu: +mu.toFixed(6), sigma: +sd.toFixed(6)
    }
  };
}
...
app.get("/api/health", (req, res) => {
  res.json({ success: true, service: "Trade Genie PRO", time: new Date().toISOString() });
});
...
app.get("/api/scrip/find", async (req, res) => {
  try {
    ...
    return res.json({
      ...
      symbol: String(symbol).toUpperCase(), exch,
      token: hit.token,
      meta: {
        name: r.name || r.symbolname || r.SymbolName || null,
        instrumenttype: r.instrumenttype || r.instrument_type || null,
...
app.get("/api/mcx/ltp", async (req, res) => {
  try {
    ...
    if (!jwtToken) {
      return res.status(401).json({ success: false, error: "Not logged in. Call /api/login first." });
    }

    const picked = (RESOLVED_MCX && RESOLVED_MCX.picked) ? RESOLVED_MCX.picked : [];
    if (!picked.length) {
      return res.json({ success: true, source: "none", data: [] });
    }

    const mcxQuotes = await fetchQuotes(jwtToken, "MCX", picked.map(p => ({ token: p.token })));
    const mapByToken = new Map((mcxQuotes || []).map(q => [String(q.token), q]));

    const out = picked.map(p => {
      const q = mapByToken.get(String(p.token));
      const ltp = (q && (q.ltp ?? q.last_price ?? q.lastPrice)) ?? null;
      return {
        label: p.label, token: p.token, symbol: p.symbol, expiry: p.expiry,
        ltp: typeof ltp === "number" ? ltp : (ltp != null ? Number(ltp) : null),
      };
    });
...
app.post("/api/backtest/montecarlo", async (req, res) => {
  try {
    ...
    return res.json({
      success: true, exchange, token: t, interval,
      S0: out.S0, params: out.params,
      stats: {
        winUp0_5: out.stats.winUp0_5, winDown0_5: out.stats.winDown0_5,
        p10: out.stats.p10, p50: out.stats.p50, p90: out.stats.p90,
        mu: out.stats.mu, sigma: out.stats.sigma
      }
    });
  } catch (e) { res.status(500).json({ success: false, error: e.message }); }
});
...
// =============================================
// MAIN ANGEL API ROUTE (login / logout / fetch_all)
// =============================================
app.post("/api/angel", async (req, res) => {
  ...
  try {
    ...
    if (action === "login") {
      ...
      if (!jwt) {
        return res.status(401).json({
          success: false, error: "Angel login failed",
    ...
    // --- FETCH ALL ---
    if (action === "fetch_all") {
      if (typeof resolveMcxTokens === 'function') resolveMcxTokens().catch(() => {});
      ...
      if (!commodities.length) {
        ...
        commodities = commodities.concat(cached);
      }
...
app.get("/", (req, res) => {
  ...
  res.status(200).send("Trade Genie PRO server is running. Place index.html in /public.");
});

// Deep routes fallback to index.html
app.get(["/dashboard", "/trade", "/stocks", "/commodities"], (req, res) => {
  const htmlPath = path.join(__dirname, "public", "index.html");
  if (fs.existsSync(htmlPath)) return res.sendFile(htmlPath);
  res.redirect("/");
});
...
// =============================================
app.listen(PORT, () => {
  console.log(`🚀 Trade Genie PRO running on port ${PORT}`);
});
