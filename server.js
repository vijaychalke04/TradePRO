...
function requestAngel(rp, method, headers, data) {
  return new Promise((resolve, reject) => {
    ...
    function attempt() {
      attempts++;
      const req = https.request({ hostname: CONFIG.baseURL, path: rp, method, headers, timeout: 25000 }, res => {
      ...
      req.on("timeout", () => req.destroy(new Error("Angel API slow (25s)")));
      req.on("error", (err) => {
        if (attempts < 2 && (err.code === "ETIMEDOUT" || err.code === "ECONNRESET" || err.code === "ECONNREFUSED")) {
          console.log(`⚠️ Angel retry ${attempts}/2: ${err.code} on ${rp}`);
...
async function fetchCachedCandles(jwt, exchange, symbolToken, interval, fromdate, todate) {
  ...
  let fresh = [];
  try { fresh = await fetchCandles(jwt, exchange, symbolToken, interval, fromdate, todate); } catch(e) { console.log(`⚠️ Candle fetch failed (${e.code||e.message}), using cache`); }
  ...
  try {
    ...
    if (action === "fetch_all") {
      ...
      const [indRaw, bseRaw, comRaw, stkRaw] = await Promise.all([
        fetchQuotes(token, "NSE", NSE_INDICES).catch(e => { console.log("⚠️ NSE idx:", e.message); return []; }),
        fetchQuotes(token, "BSE", BSE_INDICES).catch(() => []),
        fetchQuotes(token, "MCX", mcxTokens).catch(() => []),
        fetchQuotes(token, "NSE", STOCKS).catch(e => { console.log("⚠️ NSE stk:", e.message); return []; })
      ]);
...
const nseIdx = await Promise.all(indRaw.map(x => processInstrument(x, "NSE", { isIndex: true })));
      ...
      // ═══ PERSISTENT NOTIFICATION STORE + BUY/SELL TELEGRAM ═══
      if (!global._NOTIFS) global._NOTIFS = [];
      for (const al of alerts) {
        if (al.category !== "READY" && al.status !== "READY") continue;
        const nk = `${al.name}:${al.type}`;
        if (!global._NOTIFS.find(n => n.key === nk && Date.now() - n.ts < 600000))
          global._NOTIFS.unshift({ key: nk, title: `🚨 ${al.name} — ${al.label||al.type}`, body: `${al.action||""} | ₹${fmtR(al.trigger||0)} | T ₹${fmtR(al.target1||0)} | SL ₹${fmtR(al.sl||0)} | ${al.optionHint||""}`, type: "alert", ts: Date.now() });
      }
      const _TCD = global._TG_TRADE_CD || (global._TG_TRADE_CD = {});
      for (const it of [].concat(indices||[], stocks||[], commodities||[])) {
        const sig = (it.engine?.signal||"").toUpperCase();
        if (sig !== "BUY" && sig !== "SELL") continue;
        const ck = `trade:${it.name}:${sig}`;
        if (_TCD[ck] && Date.now() - _TCD[ck] < 600000) continue;
        _TCD[ck] = Date.now();
        const e = it.engine||{};
        sendTg(`📊 ${sig} — ${it.name}\n${e.trade||sig}\nOpt: ${e.optionSuggestion?.suggested||"—"}\n₹${it.ltp} | T ₹${e.projectedTarget||"—"} | SL ₹${e.stopLoss||"—"}\nConf: ${e.confPct||50}%`);
        global._NOTIFS.unshift({ key: ck, title: `📊 ${sig} — ${it.name}`, body: `${e.trade||sig} | ₹${it.ltp} | T ₹${e.projectedTarget||"—"} | SL ₹${e.stopLoss||"—"} | ${e.optionSuggestion?.suggested||""}`, type: sig.toLowerCase(), ts: Date.now() });
      }
