...
function enrichEngine(market, engine){
  ...
  const isIndex = (Array.isArray(IND_TOKENS) && IND_TOKENS.includes(String(market?.token))) || (Array.isArray(NSE_INDICES) && NSE_INDICES.includes(String(market?.token)));
...
app.post("/api/backtest/montecarlo", async(req,res)=>{
  try{
    ...
    if(rets.length < 20) return res.json({ success:false, error:"Not enough returns" });
    ...
    const nPaths = Math.min(5000, Math.max(100, parseInt(paths,10) || 1000));

    const threshold = S0 * 1.005;
    ...
    return res.json({
      ...
      stats: {
        winProbAbovePlus0_5pct: +(wins/nPaths*100).toFixed(2),
